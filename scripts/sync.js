#!/usr/bin/env node
/**
 * Incremental sync script.
 *
 * Usage:
 *   node scripts/sync.js [batchSize]
 *
 * Config from .env:
 *   GITHUB_TOKEN, GITHUB_OWNER, GITHUB_REPO, GITHUB_WORKFLOWS
 *   SYNC_BATCH_SIZE (default 10)
 *   SYNC_DATA_DIR   (default ./data)
 *
 * Each run fetches up to batchSize NEW items, resuming from the last
 * scanned page. Already-fetched run IDs are persisted and skipped.
 */

require('dotenv').config();

const GitHubActionsService = require('../services/githubActionsService');
const FileSearchService   = require('../services/fileSearchService');
const SyncManager          = require('../services/syncManager');

// ── Config ────────────────────────────────────────────────────────────────
const TOKEN     = process.env.GITHUB_TOKEN || '';
const OWNER     = process.env.GITHUB_OWNER || 'xuhongming251';
const REPO      = process.env.GITHUB_REPO || 'upload_cloud_storage';
const WORKFLOWS = (process.env.GITHUB_WORKFLOWS || 'upload.yml')
  .split(',')
  .map(s => s.trim())
  .filter(Boolean);
const WORKFLOW_EVENT = process.env.GITHUB_WORKFLOW_EVENT || 'workflow_dispatch';
const BATCH_SIZE = parseInt(process.argv[2], 10) ||
                   parseInt(process.env.SYNC_BATCH_SIZE, 10) ||
                   10;
const DATA_DIR  = process.env.SYNC_DATA_DIR || './data';

if (!TOKEN) {
  console.error('ERROR: GITHUB_TOKEN is not set in .env');
  process.exit(1);
}

// ── Main ──────────────────────────────────────────────────────────────────
(async () => {
  console.log('═══════════════════════════════════════════');
  console.log('  ModelHub — Incremental Sync');
  console.log('═══════════════════════════════════════════');
  console.log(`  Repo:      ${OWNER}/${REPO}`);
  console.log(`  Workflows: ${WORKFLOWS.join(', ')}`);
  console.log(`  Batch:     ${BATCH_SIZE} new items per run`);
  console.log(`  Data dir:  ${DATA_DIR}`);
  console.log('═══════════════════════════════════════════\n');

  const githubService = new GitHubActionsService(TOKEN, OWNER, REPO, { workflowEvent: WORKFLOW_EVENT });
  const searchService = new FileSearchService(githubService);
  const syncManager   = new SyncManager(githubService, searchService, { dataDir: DATA_DIR });

  syncManager.load();

  console.log(`Cache file: ${syncManager.cachePath}`);
  console.log(`State: ${syncManager.getItemCount()} cached items, lastRunId=${syncManager.getLastRunId()}\n`);

  const result = await syncManager.syncNewBatch(WORKFLOWS, BATCH_SIZE);

  console.log('\n═══════════════════════════════════════════');
  console.log(`  Done: ${result.newCount} new items synced`);
  console.log(`  Total cached: ${result.totalItems}`);
  if (result.done) console.log('  No more new runs found.');
  console.log('═══════════════════════════════════════════');
})();
